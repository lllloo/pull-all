...
顯示符號定義：
- `⇡N` 表示本地領先 remote N 個 commit
- `⇣N` 表示本地落後 remote N 個 commit
...
#### Scenario: current branch 領先 remote
- **WHEN** `git rev-list HEAD --not --remotes --count` 回傳 N > 0
- **THEN** 顯示「<repo>  ⇡N  (<current-branch>)」
...
#### Scenario: origin/HEAD 已設
- **WHEN** `git symbolic-ref --short refs/remotes/origin/HEAD` 成功
- **THEN** 使用其指向的 branch 名稱作為 default branch

#### Scenario: origin/HEAD 未設但 main 存在
- **WHEN** `origin/HEAD` 偵測失敗，但 `refs/heads/main` 可被 `git rev-parse --verify` 驗證
- **THEN** 使用 `main` 作為 default branch

#### Scenario: 僅 master 存在
- **WHEN** `origin/HEAD` 與 `refs/heads/main` 皆失敗，但 `refs/heads/master` 可被驗證
- **THEN** 使用 `master` 作為 default branch

#### Scenario: 三者皆不存在
- **WHEN** 三項偵測皆失敗
- **THEN** 工具 SHALL 視該 repo 為「無預設分支」，由「顯示各 repo 狀態摘要」之相應 scenario 處理
